 ...
 *
 * ⚠ **设计范围（2026-09-23 用户裁决）**：常规文档 = **≤200 页 / ≤2,000 个 `#doc` 直接子元素 / ≤1MB**。
 * 实测：200 页的常规形态只有 367KB、**1,726 块** ⇒ **在目标区间内本判据恒为"保留"**（不触发），
 * 打开代价 0、滚动收益 0。**>200 页 / >2,000 块 / >2MB 明确不在设计范围** ——
 * 不要为它们加块数上界或特判（用户明确否决了加"上界"）。预算与复跑方法见 `AGENTS.md`。
 */
...
export interface DocShape {
  ...
  blocks: number;  /** 是否含 KaTeX / Mermaid —— 渲染代价高、跳过它们的收益大于每帧判定成本 */
  heavy: boolean;
}
