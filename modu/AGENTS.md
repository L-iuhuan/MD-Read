...
- ⭐ **种入受信目录必须是【canonical 小写 ＋ `\\?\` 前缀】形态** ✗⇒✓（2026-09-23 **复现两次** ✗）：
  `\\?\d:\files\projects\md-read\.verify\phase4\ws-accept` ✓ —— 写成 `D:\Files\…` 这种原始形态 ✗ ⇒
  `dirs` 里存的是小写 `\\?\` 键 ⇒ **匹配不上** ⇒ 应用**一律拒绝**（**看起来像产品缺陷** ✗，实测两次都被误导）。
  ⇒ 种入后用 `realpathSync.native(p).toLowerCase()` 与自己的键**逐字比对**自证 ✓（别只看"我写进去了" ✓）。
- ⭐ **真机验收必须整体放进【同一条长命命令】** ✓（2026-09-23 实测两次 ✗，A/B 各 1 次 ＋ 历史 2 次同形 ✓）：
  **启动 ⇒ 等就绪 ⇒ 正对照/断言 ⇒ 截图 ⇒ 优雅关闭** 全在**一次** `pwsh` 调用内完成 ✓。
  ⚠ 否则**发起它的那一轮结束时**，harness 的 **Windows Job 回收进程树** ⇒ 探针起的应用被**静默带走** ✗
  （签名：进程消失但**无 `[child-exit]`** ✗、事件日志**无崩溃条目** ✗ ⇒ 是"被请走"不是"自己崩" ✓）。
  ⇒ 判据要成对：**同一条长命命令内 21s 全程存活 ＋ 正常 `[child-exit] code=0`** ✓ 对 **后台作业起 ⇒ 13s 内消失、无该行** ✗。
  ⇒ 推论：**任何"应用自己退出了"的怀疑，先排除这一条** ✓（本项目已因此白查两轮 ✗）。
- ⚠ **扫进程时，扫描器自己的命令行也在"干草堆"里** ✗（本会话**第 3 次**遇到）：命令行里出现
  `run-isolated` 字样，可能只是**我们自己的命令文本被嵌进 harness runner 的命令行** ✓（不是被测进程 ✓）。
  ⇒ 对策（任一条即够 ✓）：① **模式由片段拼出** ✓（避免自匹配）② **排除自己的进程树** ✓
  ③ **打印完整命令行再逐个分类** ✓（PID/父 PID/创建时刻 ✓）。
