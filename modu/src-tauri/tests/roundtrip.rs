...
use modu_lib::fs::{read_file, save_file};
...
fn read(path: &std::path::Path) -> modu_lib::fs::LoadedFile {
    read_file(path.to_string_lossy().into_owned()).expect("read_file 不应失败")
}

fn save(path: &std::path::Path, text: String, encoding: String, bom: bool) {
    save_file(path.to_string_lossy().into_owned(), text, encoding, bom)
...
fn utf8_bom_crlf_roundtrip() {
    ...
    save(&path, loaded.text, loaded.encoding, true);
    assert_eq!(
...
fn utf8_nobom_lf_roundtrip() {
    ...
    save(&path, loaded.text, loaded.encoding, false);
    assert_eq!(
        std::fs::read(&path).expect("读回应成功").as_slice(),
        text.as_bytes(),
        "无 BOM + LF 往返应字节一致"
...
fn gb18030_crlf_roundtrip_and_bom_ignored() {
    ...
    save(&path, loaded.text.clone(), loaded.encoding.clone(), false);
    assert_eq!(
        std::fs::read(&path).expect("读回应成功").as_slice(),
        original.as_ref(),
        "GB18030 往返应与原文件字节一致"
